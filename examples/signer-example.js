...
(async () => {
  ...
  const hznjs = synthetix({ network: "kovan", signer, provider });
  const { formatEther, parseEther, parseUnits } = hznjs.utils;

  console.log(
    "Remaining ETH capacity in the EtherWrapper (kovan):",
    formatEther(await hznjs.contracts.EtherWrapper.capacity())
  );
